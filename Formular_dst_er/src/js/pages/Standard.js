...
export default class Home extends React.Component {
    ...
    handleSubmit = () => {
        ...
        if (errors.vname == false && errors.nname == false && errors.hnr == false && errors.plz == false && errors.str == false && errors.ort == false) {
            ...
            this.setState({
                ...
                this.props.dispatch(fetchClickData('home', data, performance.now(), "geld"))
            })
        } else {
    ...
    render() {
        ...
        form[0] = {
            id: 1,
            index: "",
            html: <Form.Group widths='equal'><Form.Input id="1" label='Vorname' placeholder='Vorname' name='vname'
                                                         value={vname} onKeyUp={this.handleKeyUp}
                                                         onClick={this.handleClick} onChange={this.handleChange}
                                                         error={errors.vname ? "error" : ""}/></Form.Group>
        }
        form[1] = {
            id: 2,
            index: "",
            html: <Form.Group widths='equal'><Form.Input id="2" label='Nachname' placeholder='Nachname' name='nname'
                                                         value={nname} onKeyUp={this.handleKeyUp}
                                                         onClick={this.handleClick} onChange={this.handleChange}
                                                         error={errors.nname ? "error" : ""}/></Form.Group>
        }
        form[2] = {
            id: 3,
            index: "",
            html: <Form.Group widths='equal'><Form.Input id="3" label='Straße' placeholder='Straße' name='str'
                                                         value={str} onKeyUp={this.handleKeyUp}
                                                         onClick={this.handleClick} onChange={this.handleChange}
                                                         error={errors.str ? "error" : ""}/></Form.Group>
        }
        form[3] = {
            id: 4,
            index: "",
